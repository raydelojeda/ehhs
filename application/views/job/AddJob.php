                            ...
                            <div class="col-sm-12">
                                        ...
                                        <select name="sel_employee" id="sel_employee" class="my_select2_employee required_select" style="width: 100%;">
                                            ...
                                            if(isset($worker['data']))
                                            {
                                                ...
                                                foreach ($worker['data'] as $row)
                                                {
                                                    ...
                                                    <option <?php if($row->id_employee==$id_employee)print 'selected';?> value="<?php print $row->id_employee;?>" id="<?php print $row->id_employee;?>" title="<?php print $row->id_person;?>"><?php print $row->first_name.' '.$row->second_name.' '.$row->last_name.' - '.$row->zip;?></option>
                                ...
                                <div class="row" style="margin-bottom: 20px;">
                                    <fieldset class="myfieldset" style="margin-top: 5px;padding: 0px;">
                                        ...
                                        <table id="data_table_assigned_job" class="table table-condensed table-responsive table-striped table-hover " style="margin-left: auto;margin-right: auto;" width="100%"></table>
    ...
    jQuery(document).ready(function()
    {
        ...
        function LoadTableJob(id_employee='')
        {
            ...
            jQuery.ajax({
                ...
                type: 'POST',
                data: {data_type:'data_list_assigned_job',view_url:'job/DataTableListAssignedJob', id_employee:id_employee}
            }).done(function(response, textStatus, jqXHR)
            {
                if(response)
                {
                    if(jQuery('#data_table_assigned_job').html()!='')jQuery("#data_table_assigned_job").dataTable().fnDestroy();
                    jQuery('#data_table_assigned_job').html(response);
                    DataTable_Job();
                    spinner.stop();
                }
            });
        ...
        {
            var data_table_assigned_job=jQuery('#data_table_assigned_job').DataTable(
                {
                    dom:
            ...
            return data_table_assigned_job;
        }
