...
if(isset($data['client']['data']))
{
    ...
    foreach ($data['client']['data'] as $row)
    {
        ...
        <tr id="<?php print "tr" . $i;?>">
            ...
            <td class="text-center <?php if(isset($row->id_client) && isset($row->status) && $row->status=='1') print 'add_care';?>" id="<?php if(isset($row->id_client))print $row->id_client;?>" data-goto="general-Update&<?php print $row->id_user.'-'.$row->id_person;?>"><?php if(isset($row->id_client) && isset($row->status) && $row->status=='1') print '<span class="fa fa-plus-circle"></span>';?></td>
            <td class="text-center"><input name='cbx' type='checkbox' data-goto="<?php print $row->id_user;?>" id="<?php if(isset($row->id_client))print $row->id_client;?>" class="cbx_row"></td>
