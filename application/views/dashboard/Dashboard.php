...
<div class="container content">
    ...
    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 text-center" style="padding: 0px;">
        ...
        {//var_dump($approved['data']);
            if($available_care['error_code']=='0' && $approved['error_code']=='0' && $approved['data'][0]->approved=='1')$total_available_care=sizeof($available_care['data']);//else $total_available_care='Your account is not approved';
            ?>

            <?php if($approved['error_code']=='0' && $approved['data'][0]->approved=='1'){?><a onclick="LoadContent('Main/GoView/job-ListJob');"><?php }?>
                <div class="col-sm-4" style="margin-bottom: 10px;">
                    <div class="servive-block servive-block-default" style="height: 150px;">
                        <span class="badge badge-green" style="font-size: 35px; width: 75px;padding-top: 20px;padding-bottom: 20px;border-radius: 50% !important;">
                            <div class="count">
            ...
            <?php if($approved['error_code']=='0' && $approved['data'][0]->approved=='1'){?></a><?php }?>
            ...
            if($badge!='')
            {
                ...
                <a onclick="LoadContent('User');">
                    <div class="col-sm-4" style="margin-bottom: 10px;">
                        ...
                        <?php } else {?>
                            <i class="fa fa-thumbs-up" style="color:green;font-size: 72px;"></i>
                        <?php }?>
...
<script type="text/javascript">
    ...
    {
        if('<?php if(isset($session['rol'])) print $session['rol'];?>'=='asist')
        {
            jQuery('#myslider').hide();
            jQuery('html, body').animate({
                    scrollTop: jQuery(".topbar").offset().top-100});
        }
        else
            jQuery('#myslider').show(5);
